...
public class AdminAttendanceController {
    ...
    public ApiResponse<Map<String, Object>> summary(@RequestParam(required = false) Integer year,
        ...
        List<Map<String, Object>> topOrganizers = Rows.list(jdbc,
                "select organizer,count(*) count from attendance_events where (? is null or (event_date>=? and event_date<?)) and (?='' or activity_region=?) and coalesce(organizer,'')<>'' group by organizer order by count desc limit 3", year, start, end, r, r);
